...
export default function EstoqueTable() {
  ...
  async function pegarDados() {
    ...
    if (franquia != undefined) {
      ...
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          const soFiliais = data.slice(1);
          setFiliais(soFiliais);
        } else {
          alert('error filial');
        }
      } catch (error) {
  ...
  return (
    <div className="container">
      ...
      >
        <i className="bi bi-arrow-left ms-1"></i> Voltar para matriz
      </button>
      <h2 className="titulo">
        <BarChart3 className="iconeTitulo" size={22} />
        <span className="titulo-preto">Controle de</span>
        <span className="titulo-verde"> Estoque das filiais:</span>
      </h2>
